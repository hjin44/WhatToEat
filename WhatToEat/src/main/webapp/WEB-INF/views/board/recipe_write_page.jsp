...
function formSubmit(thisBox){
   ...
   //이미지 검사
   if(!cookListClass[0].childNodes[1].childNodes[1].value){
      alert("요리순서에 이미지를 첨부해주세요");
      cookListClass[0].childNodes[1].childNodes[1].focus();
      return;
   }
   
   if(!fileForm.test(cookListClass[0].childNodes[1].childNodes[1].value)){
      alert("사진파일만 첨부가능합니다");
      cookListClass[0].childNodes[1].childNodes[1].focus();
      return;
   }
   //내용검사
   if(!cookListClass[0].childNodes[3].childNodes[0].value){
      alert("요리순서에 내용을 작성해주세요");
      cookListClass[0].childNodes[3].childNodes[0].focus();
      return;
   }
   
   for(var i = 1; i < cookListClass.length; i++){
      //이미지검사
      var imageBox = cookListClass[i].childNodes[0].childNodes[0];
      var image = imageBox.value;
      ...
      //내용검사
      var contentBox = cookListClass[i].childNodes[1].childNodes[0];
   ...
   xmlhttp.onreadystatechange = function(){
      if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
         if(xmlhttp.responseText == ""){
            ...
            groupBox.selectedIndex = 10; // 대분류에 없는재료 추가 선택
...
function cookListPlus(){
   ...
   colButtonBox.setAttribute("class","col-1");
      ...
      buttonBox.setAttribute("style","background-color:transparent;border:none;outline:none;color:gray;");
      buttonBox.setAttribute("onclick","listDelete(this)");
      buttonBox.innerText = "x";
...
<ul class="fb__header">
  ...
  <li>
    <div>
    <div id="test_1" class="fb__font1" onclick="xxxx()">레시피 작성</div>
    </div>
  </li> 
  <li>
    <div>
...
<div class="container" style="width:1130px;"><!-- 현정6 -->
    <div class="row">
      ...
      <form id="form_recipe" action="${pageContext.request.contextPath}/board/recipe_write_process.do" method="post" enctype="multipart/form-data">   
         <div class="row mt-3 pt-4 pb-4 ps-3 pe-4 border border-3" style="background-color:#f2f2f2;border-radius:10px">
            <div class="col"><!-- 현정6 위에 mt-3 -->
               <div class="row">
                  ...
                  <div class="col"><input id="input_title" class="form-control" type="text" name="recipe_title" placeholder="예) 김치 안 넣은 김치 볶음밥"></div>
               </div>
               <div class="row mt-4">   
                  <div class="col-2">카테고리</div>
                  <div class="col">
                     <select id="input_national" class="form-select" aria-label="Default select example" name="national_no">
                        <option selected>나라별 전체</option>
               ...
               <div class="row mt-4">
                  ...
                  <div class="col"><input id="input_sauce" class="form-control" name="recipe_sauce" type="text" placeholder="양념"></div>
               </div>
               <div class="row mt-2">
                  <div class="col-2"></div>
                  <div class="col"><input id="input_people" class="form-control" name="recipe_people" type="text" placeholder="인원"></div>
                  <div class="col-1 mt-2 text-start" style="padding:0">인분</div>
                  <div class="col"><input id="input_time" class="form-control" name="recipe_time" type="text" placeholder="소요 시간"></div>
                  <div class="col-1 mt-2 text-start" style="padding:0">분</div>
               </div>   
            </div>
            
            <div id="thumbnail_col" class="col-4 border border-1" style="background-color:white;border-radius:5px; height:300px;"><!-- 대표 사진 -->
               <input  id="input_thumbnail" name="thumbnail_file" type="file" accept="image/*" onchange="setThumbnail(event);">
               <div id="thumbnail_box"></div>
            </div>
         </div>
   ...
   <div class="col">
      ...
      <div class="ingredient_list">
         <div class="row mt-2">
            ...
            <div class="col">
               <input class="form-control" type="text" name="ingredient_recipe_quantitys" placeholder="재료의 양">
            </div>
            <div class="col-1"></div>
         </div>
         <div class="row"></div>
   ...
   <div class="col">
      <div id="cook_list"><!-- 자바스크립트로 요리순서 추가 하려고 만든 박스 ----------------------------->
         ...
         <div class="cookList deleteListBox row mt-2 ps-3">
            ...
            <div class="col"><textarea name="recipe_contents" class="form-control" rows="10" placeholder="예) 달구지 않은  팬에 식용유와 파를 넣고 노릇노릇 하게 될 때까지 볶아주세요"></textarea></div>
            <div class="col-1"></div>
         </div>
      </div>   
         ...
         <div class="row mt-5 mb-5" style="text-align:right;">
            <div class="col">
               <input onclick="formSubmit(this)" class="btn btn-registBtn" type="button" value="저장">
...
<div class="d-none"><!-- ------------------------------------------------------ -->
    <div id="ingredient_list_template" class="deleteListBox ingredient_list">
      <div class="row mt-2">
         ...
         <div class="col-1">
            <button type="button" onclick="listDelete(this)" style="background-color:transparent;border:none;outline:none;color:gray;">x</button>
         </div>
      </div>
      <div class="row"></div>
   </div>
